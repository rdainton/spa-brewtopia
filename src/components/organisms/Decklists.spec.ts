...
describe('organisms/Decklists.vue', () => {
  test('renders message if no decklists found', async () => {
    // const wrapper = shallowMount(Decklists)
    // // mock api call
    // // currently use of the router in the axios interceptor is causing an error.
    // // return []
    // expect(wrapper.text()).toContain('No decklists found.')
    return
  })
})
